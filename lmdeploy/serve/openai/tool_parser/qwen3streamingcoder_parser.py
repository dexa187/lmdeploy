...
@ToolParserManager.register_module(['qwen3streamingcoder'])
class Qwen3StreamingCoderToolParser(ToolParser):
    """Streaming-optimized parser for Qwen3 Coder model's tool call format.

    Functionally compatible with :class:`Qwen3CoderToolParser`, but emits tool
    ``arguments`` incrementally as the model generates them (true token-level
    streaming of large parameter values) instead of buffering the whole call.

    Handles Qwen3 Coder's output format, which uses purely XML tags for function
    names and parameters, e.g., <tool_call> <function=func_name>
    <parameter=arg_name>arg_value</parameter> </function> </tool_call>
    """
    ...
    def _extract_params(self, content: str) -> tuple[str | None, dict[str, Any], bool]:
        ...
        while True:
            ...
            args_dict[param_name] = self._parse_value_string(param_val_str)
    ...
    def extract_tool_calls_streaming(
        ...
        # Keep the original API-server contract: always return a ``DeltaMessage``.
        # ``content`` stays ``None`` while inside a tool block so the raw XML
        # tokens are suppressed by the caller.
        if text_parts:
